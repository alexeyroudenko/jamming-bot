...
export default function AtlasTimeRhythm({ steps }) {
  ...
  useEffect(() => {
    ...
    try {
      let x;
      if (useTime) {
        const ext = d3.extent(series, (d) => new Date(d._ts));
        if (!ext[0] || !ext[1] || Number.isNaN(+ext[0]) || Number.isNaN(+ext[1])) {
          throw new Error('Invalid timestamp domain');
        }
        x = d3.scaleTime().domain(ext).range([0, innerW]);
      } else {
        x = d3.scaleLinear().domain(d3.extent(series, (d) => d._n)).range([0, innerW]);
      }

      const maxLen = d3.max(series, (d) => d.text_length ?? 0) || 1;
      const y = d3
        .scaleLinear()
        .domain([0, maxLen])
        .nice()
      ...
      g.selectAll('rect.bar')
        ...
        .attr('y', (d) => y(d.text_length ?? 0))
        .attr('height', (d) => innerH - y(d.text_length ?? 0))
        .attr('rx', 1)
      ...
      console.error('AtlasTimeRhythm chart error', err);
      d3.select(el).selectAll('*').remove();
      d3.select(el)
        .append('svg')
        .attr('width', width)
        ...
        .attr('font-size', 12)
        .text('Time/rhythm chart error (see console)');
    }
  }, [steps]);
