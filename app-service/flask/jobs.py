...
SCREENSHOT_RENDER_RETRIES = max(1, int(os.getenv("SCREENSHOT_RENDER_RETRIES", "2")))
...
# Cap renderer HTTP timeout so (retries × timeout + backoffs) stays under RQ ceiling minus S3 slack.
HTTP_SCREENSHOT_RENDER_TIMEOUT_RAW_SEC = int(os.getenv("HTTP_SCREENSHOT_RENDER_TIMEOUT_SEC", "5"))
SCREENSHOT_PIPELINE_S3_SLACK_SEC = float(os.getenv("SCREENSHOT_PIPELINE_S3_SLACK_SEC", "4"))
_sr_budget = float(RQ_JOB_TIMEOUT_PIPELINE_SEC) - SCREENSHOT_PIPELINE_S3_SLACK_SEC
_sr_backoff_total = max(0, SCREENSHOT_RENDER_RETRIES - 1) * SCREENSHOT_RENDER_BACKOFF_SEC
_sr_http_cap = max(
    1,
    int((_sr_budget - _sr_backoff_total) / float(SCREENSHOT_RENDER_RETRIES)),
)
HTTP_SCREENSHOT_RENDER_TIMEOUT_SEC = min(HTTP_SCREENSHOT_RENDER_TIMEOUT_RAW_SEC, _sr_http_cap)
_SCREENSHOT_HTTP_TIMEOUT_CLAMPED = HTTP_SCREENSHOT_RENDER_TIMEOUT_RAW_SEC > HTTP_SCREENSHOT_RENDER_TIMEOUT_SEC
...
if _SCREENSHOT_HTTP_TIMEOUT_CLAMPED:
    logger.warning(
        "HTTP_SCREENSHOT_RENDER_TIMEOUT_SEC capped from %ss to %ss "
        "(RQ_JOB_TIMEOUT_PIPELINE_SEC=%s, SCREENSHOT_PIPELINE_S3_SLACK_SEC=%s, retries=%s)",
        HTTP_SCREENSHOT_RENDER_TIMEOUT_RAW_SEC,
        HTTP_SCREENSHOT_RENDER_TIMEOUT_SEC,
        RQ_JOB_TIMEOUT_PIPELINE_SEC,
        SCREENSHOT_PIPELINE_S3_SLACK_SEC,
        SCREENSHOT_RENDER_RETRIES,
    )
