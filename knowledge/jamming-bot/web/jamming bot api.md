...
Публичный эндпоинт (префикс `/bot/`, без логина). Если inject уже идёт, новый запрос **вытесняет** предыдущий: сценарий сбрасывается и стартует заново с новым текстом (ответ `202`, поле `supersedes` — id предыдущего inject).
