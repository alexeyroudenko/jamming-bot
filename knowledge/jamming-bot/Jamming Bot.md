---
title: Jamming Bot
note_type: project
status: active
version: "2.1.0"
years: 2023-2026
url: https://jamming-bot.arthew0.online/
aliases:
  - Jamming bot
owner:
  - "[[arthew0]]"
soft:
  - "[[TouchDesigner]]"
technology_stack:
  - "[[Python]]"
  - "[[Docker]]"
  - "[[Spacy]]"
  - "[[Flask]]"
  - "[[Redis]]"
  - "[[Redis Queue]]"
  - "[[socket.io]]"
  - "[[nginx]]"
  - "[[postgres]]"
  - "[[sqlite]]"
  - "[[React JS]]"
  - "[[HTML]]"
  - "[[css]]"
  - "[[D3]]"
  - "[[Portainer]]"
  - "[[fastapi]]"
tags:
  - project
  - art
  - jamming-bot
---

# Jamming Bot

![[Jamming Bot - Scene 01 - Path.png]]

Карта содержания: [[Jamming Bot MOC]]

## Концепция

В настоящее время [[пауки]], [[краулеры]] и боты несут сервисную цель. Они выступают в роли поисковиков, собирают информацию и автоматизируют инфраструктуры интернета. Jamming Bot - это фантазия на тему постапокалиптического будущего, когда основные функции интернета и ботов-помощников будут повержены и останется лишь один самовоспроизводящийся бот, бесцельно бороздящий просторы интернета и, возможно, изучающий в обрывках смыслов на страницах интернета общее настроение человечества. Это [[бот]], у которого нет цели, а только [[путь]].

### Философия и нарратив «пути»

- [[Вырыпаев — Путь домой]] — литературный слой (отрывок из «Сахар»).
- [[Voitenkov — Философия постмодерна (транскрипт лекции)]] — постмодерн и кризис субъекта/объекта (транскрипт лекции).
- [[Ницше — смерть Бога (заметка vault)]], [[Свобода и самоопределение (заметка vault)]] — опорные карточки по формуле и свободе.

### «Jamming» в названии

Имеется в виду не радиопомехи, а **музыкальный jam**: сессия, поток, настрой без внешней «миссии». Культурный слой — оригинал **Bob Marley & The Wailers — «Jamming»** (*Exodus*, 1977) и, в первую очередь, **кавер 5'nizza** (часто в списках как «To Bob Marley (Jamming)» / «Jammin'», альбом *Анплаггед* / *Unplugged*): акустическая, «уличная» подача, через которую у многих в постсоветском контексте слово *jamming* цепляется к образу именно так. Это перекликается с идеей бота без цели — только путь. Слушать и держать рядом с проектом: [[Jamming bot музыкальная подборка]].

[[Jamming bot экспликация]]

## Быстрые входы

- [[Jamming Bot MOC]] - полная карта содержимого проекта
- [[Jamming bot web|Jamming Bot Web]] - карта веб-поверхности и интерфейсов
- [[dev/_index|Jamming Bot Dev Index]] - архитектура, инфраструктура, задачи и исследования
- [[Jamming Bot - Theory Context]] - теоретическая рамка проекта
- [[exhibitions/_index|Выставки]] - материалы показов и экспликаций

## Ключевые блоки проекта

- Сцены: [[Scene 1. Path]], [[Scene 2. Dependency Parsing]], [[Scene 3. Texts]], [[Scene 4. Geo]], [[Scene 5. Cloud]], [[Scene 6. Embeddings]]
- Нарратив: [[Jamming bot AV]], [[Jamming bot экспликация]], [[Jamming bot scenes]], [[jamming_bot_en]], [[Вырыпаев — Путь домой]], [[Voitenkov — Философия постмодерна (транскрипт лекции)]]
- Разработка: [[Jamming bot Dev project|Jamming Bot Dev Project]], [[Jamming bot microservices|Jamming Bot Microservices]], [[Jamming bot Tasks|Jamming Bot Tasks]]
- Архив: [[Jamming Bot v1.0.0]], [[Jamming_bot_v1.0]]

## Текущая роль заметки

- Каноническая карточка проекта: что это, зачем, текущий статус.
- Подробная навигация и длинные перечни вынесены в [[Jamming Bot MOC]].

## Связанные идеи

- [[Идеи - Карта пути]]
- [[Вырыпаев — Путь домой]]