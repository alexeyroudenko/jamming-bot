# semantic-cloud-words-combinator
...
#### Scenario: Slice uses array order

- **WHEN** the tag cloud is rendered
- **THEN** the consecutive phrase SHALL be chosen by indices in the `tags` array; visual order in the cloud MAY differ if the cloud component shuffles layout
...
The system SHALL retain only the last three phrase lines in state. On unmount, the phrase timer SHALL be cancelled so no further ticks run.
